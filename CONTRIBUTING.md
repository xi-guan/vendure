...
All packages in this repo are released at every version change using a fixed versioning scheme. This simplifies both the development (tracking multiple disparate versions is tough) and also the developer experience for users of the framework (it is simple to see that all packages are up-to-date and compatible).
...
# Then build all packages to ensure the distributed files are up to date
npm run build
...
Although we are using [conventional commits](https://www.conventionalcommits.org), the version is not automatically being calculated from the commit messages. Therefore, the next version should be manually selected.

The changelog generation will create changelog entries for this release.
...
Finally, a GitHub release should be created based on the tag. This will trigger the "Publish to npmjs" workflow
